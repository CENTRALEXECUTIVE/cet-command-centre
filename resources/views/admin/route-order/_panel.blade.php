{{-- Shared route-order panel. Expects: $scope, $tabs, $selected, $rows, and
     $panelRoute (the route name to link the tabs back to). --}}
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
    @foreach(['upcoming' => 'Upcoming', 'today' => 'Today', 'past' => 'Past 60d', 'all' => 'All (60d)'] as $key => $label)
        <a href="{{ route($panelRoute, ['scope' => $key, 'route' => $selected]) }}"
...
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px">
    @foreach($tabs as $t)
        <a href="{{ route($panelRoute, ['scope' => $scope, 'route' => $t['tag']]) }}"
