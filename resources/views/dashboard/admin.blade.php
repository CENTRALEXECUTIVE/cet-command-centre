...
@section('content')
    ...
    <div class="deck">
        ...
        <a class="kpi {{ $bookedTodayCount > 0 ? 'ok' : '' }}" href="{{ route('bookings.index', ['filter' => 'booked-today']) }}">
            <div class="kpi-ico">📥</div>
            <div class="kpi-n" data-countup="{{ $bookedTodayCount }}">{{ $bookedTodayCount }}</div>
            <div class="kpi-l">Bookings taken today</div>
        </a>
        <a class="kpi {{ $bookedTodayRevenue > 0 ? 'ok' : '' }}" href="{{ route('bookings.index', ['filter' => 'booked-today']) }}">
            <div class="kpi-ico">📈</div>
    ...
    {{-- Control-tower log: live watchdog events, critical rows glow until
         acknowledged. Polls every 30s; chime per-admin preference. The big
         top-right popups + the poller/siren come from the app layout (global on
         every admin page); this panel is the full scrolling log. --}}
