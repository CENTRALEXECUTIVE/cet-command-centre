    ...
    public function upsertFromParsed(array $parsed, bool $allocateRotation = true): ?array
    {
        ...
        return DB::transaction(function () use ($parsed, $reference, $existing, $vehicleType, $pickupAt, $paymentStatus, $allocateRotation) {
            ...
            if ($existing) {
                ...
                // THE OFFICE IS THE BOSS: a routine re-ingest must NEVER overwrite
                // a field the office has changed in the app. Once a person edits
                // the pickup time (or address, flight, passengers, seats, luggage…)
                // in Command Centre, that value is final and the email only
