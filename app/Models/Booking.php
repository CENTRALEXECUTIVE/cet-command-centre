...
class Booking extends Model
{
    ...
    public function viaStops(): array
    {
        ...
        // 3) The calendar event's own words — the operator's source of truth for a
        ...
        //    it having to be duplicated into a column first. The live calendar
        //    ranks ABOVE the old ETO import text: if both exist, the calendar (the
        //    thing the operator actually maintains) wins.
        if ($fromCalendar = $this->calendarViaStops()) {
            return $fromCalendar;
        }

        // 4) The ETO CSV import's free-text "Via" field (one or more, joined) —
        //    the last-resort fallback when there's no calendar event to read.
     ...
     */
    public function emergencyCallTarget(): string
    {
        $officeDrivers = array_filter(array_map(
            fn ($e) => strtolower(trim($e)),
            explode(',', (string) config('cet.checkpoint.office_call_drivers', 'admin@centralexecutivetransfers.co.uk'))
        ));
        $email = strtolower((string) $this->driver?->email);

        return ($email !== '' && in_array($email, $officeDrivers, true)) ? 'office' : 'driver';
    }
    ...
    /**
     * The standard share of the fare the DRIVER keeps (default 90% — the company
     * takes 10% commission). Tunable via the `driver_pay_percent` setting.
     */
    public function driverPayPercent(): float
    {
        return (float) \App\Models\Setting::get('driver_pay_percent', 90);
    }

    /**
     * The STANDARD driver pay suggestion — the driver's share of the fare (default
     * 90%). Pre-filled in the payroll box so the office just clicks to confirm, or
     * types a different figure. Null when there's no fare to base it on.
     */
